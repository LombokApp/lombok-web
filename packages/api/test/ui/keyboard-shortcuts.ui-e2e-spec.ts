...
describe('UI E2E - Keyboard Shortcuts', () => {
  ...
  it.skip('should handle Ctrl/Cmd+Enter for form submission', async () => {
    // Login form does not support Ctrl/Cmd+Enter for submission
    const page = await testModule!.getFreshPage()

    await page.goto(`${testModule!.frontendBaseUrl}/login`)
    await page.waitForLoadState()

    // Username must match /^\w+$/ (no hyphens)
    const username = `uishortcut${Date.now()}${Math.random()
      .toString(36)
      .slice(2, 8)}`
  ...
  it.skip('should handle Ctrl/Cmd+/ for help or shortcuts menu', async () => {
    // App does not have a Cmd+/ help or shortcuts dialog
    const page = await testModule!.getFreshPage()
