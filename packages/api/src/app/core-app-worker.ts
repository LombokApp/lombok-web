  ...
  if (
    ...
    void (async () => {
      if (fs.existsSync(bundleCacheRoot)) {
        console.log('Removing old bundle cache', bundleCacheRoot)
        await fsPromises.rmdir(bundleCacheRoot, { recursive: true })
      }
      await fsPromises.mkdir(bundleCacheRoot, { recursive: true })
